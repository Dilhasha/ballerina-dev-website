...
A `package` is a collection of `modules`, and a `module` is a collection of Ballerina source files, and test files. A package should contain at least one module called the default module. Each module has its own directory, which organizes source files, and test files. A package can contain resources and test-only resources specific to the package.
